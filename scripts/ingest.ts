...
const EXPORTS_DIR = path.resolve('claude-exports');
...
function findDataDirs(): string[] {
	if (!fs.existsSync(EXPORTS_DIR)) {
		console.error(`claude-exports/ directory not found at ${EXPORTS_DIR}`);
		process.exit(1);
	}

	const entries = fs.readdirSync(EXPORTS_DIR, { withFileTypes: true });

	// Extract ZIPs that haven't been extracted yet
	for (const entry of entries) {
		if (entry.isFile() && entry.name.endsWith('.zip')) {
			const zipPath = path.join(EXPORTS_DIR, entry.name);
			const dirName = entry.name.replace('.zip', '');
			const dirPath = path.join(EXPORTS_DIR, dirName);
	...
	const refreshed = fs.readdirSync(EXPORTS_DIR, { withFileTypes: true });
	return refreshed
		.filter((e) => e.isDirectory() && e.name.startsWith('data-'))
		.map((e) => path.join(EXPORTS_DIR, e.name));
}
